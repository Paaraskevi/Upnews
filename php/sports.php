...
function getAllResults($conn, $total_records_per_page, $page) {
	...
	$num_articles = ceil($total_articles / $total_records_per_page);
	
	$data=[];
	
	$data["articles"]=$res;
	$data["pages"]=$num_articles;
...
FROM testdb.news 
LEFT JOIN testdb.categories  ON news.cat = categories.id Where categories.categories='Sports' 
ORDER BY cal ASC";
